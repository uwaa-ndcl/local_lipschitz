...
def solve_LipSDP(network, lip_params):
    ...
    weights, net_dims = weight_utils.load_weights(network['weight_path'])
    
    # if splitting flag is supplied, split network into subnetworks
    if lip_params['split']:
                
        split_W, split_net_dims = weight_utils.split_weights(
            weights, net_dims, lip_params['split_size'])
        L = split_and_solve.split_and_solve(split_W, split_net_dims, lip_params, network)
    
    # otherwise, solve a single SDP for the entire network
    else:
        
        L = lipschitz_multi_layer.lipschitz_multi_layer(
            weights, lip_params['formulation'], lip_params['verbose'],
            lip_params['num_neurons'], lip_params['num_dec_vars'], net_dims, network)
